...
producciones *Rete* (la librería [Lisa](https://github.com/ldotlisa/Lisa)) con
llamadas a proveedores de LLM (OpenRouter, Anthropic, OpenAI, Gemini, Groq,
...
`max_fact_payload_chars`), para que un archivo enorme no devore el presupuesto.
...
### 5b. Streaming en vivo (opencode-style)

Con `llm_stream: true` (por defecto) la respuesta del LLM no llega entera:
`stream-llm-with-tools` consume los *deltas* SSE a medida que el proveedor los
emite y los imprime en vivo, con

- el **razonamiento** del modelo (p. ej. `delta.reasoning` en DeepSeek) mientras
  piensa — visible progresivamente, pero no se acumula en el contexto,
- el **texto final** (`delta.content`) a medida que se genera,
- **banners de herramienta** antes/durante la ejecución:

```
assistant>
The user is asking how many lines are in the file src/context.lisp. ...
 ⚙ exec_command {"command": "wc -l src/context.lisp"}
  ✓ 74 chars received

El archivo `src/context.lisp` tiene **320 líneas**.
```

El canal de salida de deltas es `(llm-stream-out)`, que usa `*stream-output*`
si está definido o el `*standard-output*` actual en caso contrario (evita
streams obsoletos al arrancar desde una imagen SBCL guardada). El loop sigue
siendo multi-iteración: tras una `tool_calls` se vuelve a pedir respuesta y
esta también streamea. Los `[LLM ERROR]`/`[LLM API ERROR]` no se guardan como
hechos y fuerzan exit code 1 en modo one-shot.

`assistant>` se imprime una sola vez por turno: si el stream ya mostró el texto,
`run-one-shot` y el REPL no lo reimprimen (guard `*llm-streamed*`).
...
| `:clear`               | Vacía la memoria de hechos (working memory de Lisa)  |
| `:clearall`            | Reset completo: hechos + contadores + métricas + session id nuevo |
...
5. **Contadores monótonicos entre rebotes**: el dump persiste
   `*turn-counter*`/`*todo-counter*`/`*epoch-counter*`, y al reanudar la
   imagen (`start-harness nil`) o tras `:restore` se continuan, de modo que
   los `turn-id` (y todos/epochs) no se reutilizan ni colisionan en el
   render agrupado por turno.
...
| Herramientas amplias (edit de archivos, MCP, GitHub, LSP, web) | NO | En el loop de tool-use solo se exponen `exec_command` y `read_file`; `write-file` existe como función/comando del REPL pero no se ofrece al modelo. |
...
- **Modo TUI interactivo** (como `opencode`):
  ...
  Cuando se ejecuta desde un terminal interactivo (TTY), `run.sh` usa `rlwrap`
  automáticamente para habilitar edición de línea e historial de comandos.
...
- **Modo no interactivo, un turno** (como `opencode run "..."`): procesa el
  ...
  (streameada en vivo por defecto), salva la sesión y sale. Código de salida
  0 = OK, 1 = fallo de LLM:
